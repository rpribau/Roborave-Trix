...
short seguidorPin = 3; // Variable que contiene pin digital 2 de seguidor de línea
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(seguidorPin), detectar, HIGH);
...
void loop() {
    ...
    if (x > 290 && x < 350) {
      ...
      if (distanciaIz < 10){     // Moverse a la derecha porque hay pared a la izquierda
        motores.movimiento(300, velocidad);
      }
      
      if (distanciaDe < 10){     // Moverse a la izquierda porque hay pared a la derecha
